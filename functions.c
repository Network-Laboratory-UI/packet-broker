...
//         while (pool->queueSize == 0 && !pool->poolShutdown)
//         {
//             pthread_cond_wait(&pool->queueNotEmpty, &pool->queueMutex);
//         }

//         // If the pool is shutting down, exit the thread
...
//         Task task = pool->taskQueue[--pool->queueSize];
...
//     pool->taskQueue[pool->queueSize].function = function;
//     pool->taskQueue[pool->queueSize].argument = argument;
//     ++pool->queueSize;
